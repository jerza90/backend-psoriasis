...
export default function HeroProduct() {
  ...
  const heroProducts = useMemo(() => products.filter((x) => x.isHero && x.id !== 'probiotic'), [products]);
  ...
  useEffect(() => {
    if (!heroProducts.some((x) => x.id === selectedHeroId)) {
      setSelectedHeroId(heroProducts[0]?.id ?? products[0]?.id ?? '');
    }
  }, [heroProducts, products, selectedHeroId]);
                        ...
                        >
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {active && <span className="text-[11px] font-bold uppercase tracking-widest text-green">{t('heroProduct.selected')}</span>}
                            </div>
                          </div>
